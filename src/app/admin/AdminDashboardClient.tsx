...
export default function AdminDashboardClient({ 
  ...
  const filteredGlobalOrders = useMemo(() => {
    if (globalTimeframe === 'all') return initialOrders;
  ...
  }, [initialOrders, globalTimeframe]);

  // --- 1. REVENUE TRENDS (Smart Grouping Engine) ---
  const revenueData = useMemo(() => {
    const now = new Date().getTime();
    const day = 24 * 60 * 60 * 1000;
    const trendOrders = initialOrders.filter(order => {
                ...
                return (
                  <div key={camp.id} className="relative">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="relative w-10 h-10 rounded-lg bg-gray-100 overflow-hidden border border-gray-200 shrink-0">
                        {camp.image_url ? (
                          <Image 
                            src={camp.image_url} 
                            alt={camp.name} 
