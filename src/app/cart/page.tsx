...
export default function CartPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Pre-flight campaign validation via shared engine
      ...
      toast.error(`An error occurred while verifying your cart: ${getErrorMessage(error)}`);
    } finally {
      setIsCheckingOut(false); 
    }
  };
              ...
              <div key={`${item.productId}-${item.purchaseType}`} className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 relative group">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {item.purchaseType === 'bulk' && <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-1 rounded">📦 Bulk Deal</span>}
                  ...
                  <div className="font-medium text-gray-500 text-sm">
                    {formatNaira(item.priceAtAddition)} / unit
                  </div>
                </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-24">
              ...
              <div className="space-y-4 text-sm text-gray-600 mb-6 pb-6 border-b border-gray-100">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span className="font-medium text-gray-900">{formatNaira(cartTotal)}</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="flex justify-between items-end mb-8">
                ...
                <span className="text-3xl font-black text-green-700">{formatNaira(cartTotal)}</span>
              </div>
